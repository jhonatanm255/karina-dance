      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {achievements.map((achievement, index) => (
            <div
              key={index}
              className="achievement-card bg-white lg:h-auto rounded-lg shadow-lg hover:shadow-xl transition-shadow transform hover:scale-105"
            >
              <div>
                <img
                  src={achievement.image}
                  alt={achievement.description}
                  className="w-full lg:h-96 lg:object-cover rounded-lg" // Imagen más alta
                />
              </div>
